...
from .forms import BigTradForm, ComponentsForm, StyleForm, StyleFormClient
from .models import BigTraditions, Components, PersonalStyle, PersonalStyleGroup
...
def big_trad_test(request):
    if (
        request.method != "POST"
    ):  # Email functionality to the results (making this accesible to non registered users)
        form = BigTradForm()
    else:
